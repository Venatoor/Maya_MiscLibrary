...
class bSkinSaverUI(QtWidgets.QDialog):
    def __init__(self, parent=getMayaWindow()):
        ...
        self.descLabel = QtWidgets.QLabel("(C) 2015 by Thomas Bittner, Updated by Ilyas El Mounjid in 2023",
                                          parent=self)
...
def bSkinObject(objectName, joints, weights):
    ...
    for i in range(len(joints)):
        ...
        for k in range(InfluentsArray.length()):
            ...
            if fileJointTokens[len(fileJointTokens) - 1] == sceneJointTokens[
                len(sceneJointTokens) - 1]:  # changed from joints
